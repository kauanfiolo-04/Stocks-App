...
const CountrySelect = ({ value, onChange }: { value: string, onChange: (value: string) => void }) => {
  ...
  const getFlagEmoji = (countryCode?: string): string => {
    ...
    // mapeamentos comuns (se quiser suportar 'UK' -> 'GB', etc.)
    const alias: Record<string, string> = {
      UK: "GB",
      // adicione outros aliases se precisar
    };
    const cc = alias[normalized] ?? normalized;

    // valida: deve ser exatamente duas letras A-Z
    if (!/^[A-Z]{2}$/.test(cc)) return "";

    // calcula os dois code points dos Regional Indicator Symbols
    const first = 127397 + cc.charCodeAt(0); // 'A' => 127462
