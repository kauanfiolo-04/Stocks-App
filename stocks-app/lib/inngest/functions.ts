...
export const sendSignUpEmail = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    await step.run("send-welcome-email", async () => {
      ...
      const introText = (part && "text" in part ? part.text : null) || "Thanks for joining StocksApp. You now have the tools to track markets and manke smarter moves."
...
export const sendDailyNewsEmailSummary = inngest.createFunction(
  ...
  async ({ step }) => {
    ...
    // Step #3: Sumarize these news via AI for each user
    ...
    return { succes: true, message: "Daily news summary sent successfuly!" };
  }
);